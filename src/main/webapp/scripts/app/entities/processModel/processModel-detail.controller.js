...
angular.module('artirestApp')
    .controller('ProcessModelDetailController', function ($scope, $rootScope, $state, $stateParams, $timeout, $http, $uibModal, entity, ProcessModel, ArtifactModel, Process) {
        ...
        $scope.saveEditArtifact = function (artifact) {
            var states = $scope.parseLifeCycle(artifact);
            for (var i in artifact.states) {
                if (artifact.states[i].conditions) {
                    for (var j in states) {
                        if (states[j].name == artifact.states[i].name) {
                            states[j].conditions = artifact.states[i].conditions;
                            break;
                        }
                    }
                }
            }
            artifact.states = states;
        ...
        $scope.attrTypes = ['String', 'Long', 'Integer', "Double", 'Float', 'Text', 'Date', 'Function'];

        $scope.saveArtifact = function (artifact) {
            console.log("save artifact:");
        ...
        $scope.newAttr = {name: null, type: null, comment: null, function: null};
        $scope.addAttr = function (artifact) {
            if ($scope.newAttr.name && $scope.newAttr.type) {
